...
class GoogleTTSService implements ITextToSpeechService {
  ...
  GoogleTTSService({required TexttospeechApi api, required http.Client httpClient, required String apiKey}) : _api = api, _httpClient = httpClient, _apiKey = apiKey;
  ...
  Future<Uint8List> synthesizeSpeechText(
    ...
    if (audioContent == null) throw Exception('No audio content returned');
  ...
  Future<SynthesisWithTimepointsResponse> synthesizeSpeechWithTimepoints(
    ...
    final timepoints = List.generate(parsed.length, (i) {
      final offset = Duration(microseconds: (parsed[i].timeSeconds * 1e6).round());
